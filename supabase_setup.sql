...
create table if not exists public.mc3_answers (
  id uuid default gen_random_uuid() primary key,
  user_email text unique not null references public.mc3_users(email) on delete cascade,
  goal text,
  level text,
  gender text,
  location text,
...
-- IMPORTANT: If table already exists, run this to add the unique constraint:
-- ALTER TABLE public.mc3_answers ADD CONSTRAINT mc3_answers_user_email_key UNIQUE (user_email);
